class Status():
    def __init__(self,name,sex,age):
        self.name = name
        self.sex = sex
        self.age = age
        self.guanli = []
    def __str__(self):
        msg = "学生的姓名是%s,学生的性别是%s,学生的年龄是%d"%(self.name,self.sex,self.age)
        return msg
    def address(self,xs,sex,age):#添加学生功能
        self.guanli.append(xs)
    def add
list = [] #存放名字
print("名片管理系统".center(50,"*"))
while True:
    print("1:添加名片".center(50,"*"))
    print("2:查找名片".center(50,"*"))
    print("3:修改名片".center(50,"*"))
    print("4:删除名片".center(50,"*"))
    print("5:打印名片".center(50,"*"))
    print("6:退出系统".center(50,"*"))
    num = int(input("请选择功能"))
    if num == 1:
        d = {} #空字典
        while True:
            name = input("请输入你要添加的名字")
            if len(name) > 4:
                print("太长,请重新输入")
                continue
            job = input("请输入你要添加的职位")
            if len(job) > 4:
                print("太长,请重新输入")
                continue
            phone = input("请输入手机号码")
            if len(phone) != 11 or phone.startswith("1") ==False:
                print("手机号输入有误，请重新输入")
            d["name"] = name
            d["job"] = job
            d["phone"] = phone
            #添加到列表
            list.append(d)
            print("添加成功")
            break
    elif num == 2:
        num = input("请输入你要查找的姓名")
        flag = False #假设没有咱们要找的人
        for i in list:
            if num == i["name"]:
                print("姓名:%s\n职位:%s\n电话:%s"%(i["name"],i["job"],i["phone"]))
                flag = True #找到了
                break
            if flag == False:
                print("查无此人")
    elif num == 3:
        #要改之前，你得先查到你要找的那个人
        name = input("请输入你要改的那个人的姓名")
        flag = False
        for i in list:
            if name == i["name"]:
                print("1:修改姓名")
                print("2:修改职位")
                print("3:修改电话")
                num_1 = int(input("请选择功能"))
                if num_1 == 1:
                    new_name = input("请输入新的名字")
                    i["name"] = new_name
                elif num == 2:
                    new_job = input("请输入新的职位")
                    i["job"] = new_job
                elif num == 3:
                    new_phone = input("请输入新的电话")
                    i["phone"] = new_phone
                flag = True
                break
        if flag == False:
            print("查无此人")
    elif num == 4:
        name = input("请输入你要删除的名字")
        flag = False
        for position,i in enumerate(list): #吧索引遍历出来
            if name == i["name"]:
                flag = True #找到了
                print("1:确认删除")
                print("2:取消删除")
                num_2 = int(input("请选择序号"))
                if num_2 == 1:
                    list.pop(position)#直接删除
                break
        if flag == False:
            print("查无此人")
    elif num == 5:
        print("名字\t职位\t电话")
        for i in list:
            print(" "+i["name"]+"\t "+i["job"]+"\t "+i["phone"])
    elif num == 6:
        print("您已经退出系统".center(48,"*"))
    break
